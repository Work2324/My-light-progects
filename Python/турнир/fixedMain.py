...
while True:
    ...
    if not(isJump): # Чтобы во время прыжка нельзя было передвигаться вверх или вниз
        ...
        if jumpCount >= -10:
            if jumpCount < 0:
                y += (jumpCount ** 2) / 2 # Т.к траектория прыжка проходит по параболе, для этого используем квадрат,
                # Без деления на 2 прыжок будет слишком большим
                jumpCount -= 1
            elif jumpCount >= 0 and (y-(jumpCount**2)/2)>=0:
                y -= (jumpCount**2)/2
                jumpCount -= 1
            elif (y-(jumpCount**2)/2)<0:
                jumpCount -= 1
        else:
            isJump = False
            jumpCount = 10
